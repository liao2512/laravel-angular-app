...
<div class="page-header">
        <h1>Payments / Show #{{$payment->id}}</h1>
        <form action="{{ route('payments.destroy', $payment->id) }}" method="POST" style="display: inline;" onsubmit="if(confirm('Delete? Are you sure?')) { return true } else {return false };">
            ...
            <div class="btn-group pull-right" role="group" aria-label="...">
                <a class="btn btn-warning btn-group" role="group" href="{{ route('payments.edit', $payment->id) }}"><i class="glyphicon glyphicon-edit"></i> Edit</a>
        ...
        <div class="col-md-12">
            ...
            <form action="#">
                ...
                <div class="form-group">
                     ...
                     <p class="form-control-static">{{$payment->title}}</p>
                </div>
                    <div class="form-group">
                     <label for="body">BODY</label>
                     <p class="form-control-static">{{$payment->body}}</p>
                </div>
            </form>
